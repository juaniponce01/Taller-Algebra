...
cadaEcTieneSoluc ((k, r, m):es) | mod r (mcd k m) /= 0 = False
                                | otherwise = cadaEcTieneSoluc es

tieneSolucionSimplif :: [(Int ,Int)] -> Bool
tieneSolucionSimplif [es] = True
tieneSolucionSimplif ((r1, m1):(r2, m2):es) | mcd m1 m2 /= 1 && (max r1 r2) `mod` (min r1 r2) /= 0 = False
                                            | otherwise = tieneSolucionSimplif ((r2, m2):es)

tieneSolucion :: [(Int, Int, Int)] -> Bool
tieneSolucion sist = cadaEcTieneSoluc sist && tieneSolucionSimplif (sistemaSimplifEquiv sist)

dirichletAux :: Int -> Int -> Int -> Int
dirichletAux r m k | mcd r m == 1 && esPrimo (m * k + r) = m * k + r 
                   | otherwise = dirichletAux r m (k+1)
